...
public class New_product_add_general_info extends Login_the_page {
	...
	public static void 	Create_new_BLind_fabric_product() throws InterruptedException, IOException {
		Thread.sleep(5000);
		...
		Thread.sleep(3000);
		Add_product_pageobject.productname.sendKeys(prop2.getProperty("blindfabricproductname"));
		testcase.log(Status.INFO, "Enter the  product name");
		
		
       
		String productnamenumber =prop2.getProperty("blindfabricproductname").replaceAll("[^0-9]", ""); 
		int nameInt = Integer.parseInt(productnamenumber);
		nameInt++; 
		String nameplus = prop2.getProperty("blindfabricproductname").replaceAll("[0-9]", "") + String.format("%02d", nameInt); 
		System.out.println(nameplus);
		prop2.setProperty("blindfabricproductname", nameplus);
	...
	String procat="(//*[text()='"+propertyfile().getProperty("1")+"'])[2]";
